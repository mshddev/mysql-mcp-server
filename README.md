...
| `401` on every call | The token matches no `auth_tokens` entry (body `invalid token`, and the server log has an `auth` warning with the address it saw, which is the proxy's when one fronts the server; the proxy's access log has the client. The warning is written at most once a minute per address, with `repeats_suppressed` counting the rest), or the header is missing or malformed (body `no bearer token`). Compare what the client sends with that person's entry, and check the header reads `Authorization: Bearer <token>`. |
...
```bash
cp config.example.yaml config.yaml   # one caller, alice, reading MYSQL_MCP_TOKEN_ALICE
MYSQL_MCP_TOKEN_ALICE=localsecret123 MYSQL_PASSWORD=devpassword ./mysql-mcp-server
```
